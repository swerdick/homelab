...
- **Anduril gaming LXC — Sunshine streaming + audio + desktop apps (replatform gaming half DONE)** — CT 117 (privileged Ubuntu 26.04, shared `amdgpu`) runs KWin + Steam Big Picture on the TV via KMS and streams over Sunshine/Moonlight (Vulkan h264/hevc/av1 encode + KMS capture, confirmed in the privileged CT; audio hands off to the Moonlight client during a stream and back to the TV after). This session added: SSH onboarding + a salt-idempotent unified `pseudo` password, Spotify + Discord (+ a KWin Spotify-fullscreen rule), KDE Wallet disabled, the **HDMI audio profile pinned in WirePlumber** so a Steam Big-Picture display-flap can't re-park the card and silence the TV, and graceful `steam -shutdown` via the session `ExecStop`. `setup-anduril-{session,xrdp,apps,sunshine}.yaml` under `site-anduril.yaml`.
- **Anduril LXC input — resolved (the spike "Wayland input blocker" was a mis-measurement)** — KWin in the VT-less LXC processes injected mouse *and* keyboard fine (proven live: a uinput mouse moves the cursor and lights up Big Picture tiles; the controller types into Steam's search). The earlier "libinput enumerates zero devices" was simply the `libinput` binary not being installed. The one genuinely-broken thing — the Steam Controller 2 "Puck" (`28de:1304`) trackpad-as-desktop-mouse + the recurring intro dialog — is an **upstream Valve SC2-on-Linux bug** (steam-for-linux #13185), not our config. See [[project-anduril-sc2-puck-linux-bugs]].
- **Harbor as upstream-registry proxy/cache for k3s** — `terraform/harbor/` (goharbor/harbor provider) creates a paired registry + proxy-cache project per upstream (`dockerhub`, `ghcr`, `quay`, `registry-k8s`); `setup-k3s-registries.yaml` + `k3s-registries.yaml.j2` template `/etc/rancher/k3s/registries.yaml` so containerd routes pulls through Harbor transparently (tirion CA referenced explicitly — containerd ignores the host trust store). Admin creds via SOPS through the `tf-harbor` just recipe; rollout staged (dockerhub first, uncomment the rest in `k3s_registry_mirrors` after validating). (open: the provider lacks the "serve stale on upstream down" toggle — set it per-project in the UI; Helm-chart mirroring + ORAS loose-binaries are separate active items.)
