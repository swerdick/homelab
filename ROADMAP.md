...
- **Authentik SSO/IdP deployed** — `authentik.vingilot.internal`, chart 2026.5.0 via Flux in `gondor/apps/authentik/`. External Postgres (CNPG `authentik-postgresql`, operator-default PG18 + `pg_trgm` pre-created for Authentik's migrations) + per-app Valkey (cache / sessions / Celery broker). `secret_key` + DB password + `akadmin` bootstrap creds SOPS'd in `authentik-secret.yaml`, injected via `global.env`; non-secret conn params via the `authentik.*` block (chart serializes it into `AUTHENTIK_*` env). `server.ingress` off — exposed via HTTPRoute on the vingilot Gateway (TLS terminated there); hand-managed CNPG PodMonitor. **Standalone only** — wiring SSO (forward-auth / OIDC) into existing apps (Grafana, Immich, Harbor, …) is the open follow-on.
