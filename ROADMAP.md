...
- **Tailscale remote access — native `tailscaled` subnet router + WoL on samwise** — mesh VPN for off-LAN SSH/admin access, replacing per-service Cloudflare Tunnels. **Native systemd, _not_ a k3s HelmRelease** (a deliberate reversal): tailscaled wants host networking for subnet routing + the WoL broadcast, its state file is tiny (so the SD-flash-wear worry that gated the HelmRelease — and its SSD-swap dependency — evaporated), and a host daemon survives gondor's nightly shutdown with no taint/toleration/PV dance. `ansible/playbooks/install-tailscale.yaml` (host_vars-driven; auth key SOPS'd in `host_vars/samwise/secrets.sops.yaml`; `--accept-dns=false` so resolv.conf stays on the CR1000A for the future Pi-hole bootstrap order) brings samwise up as a **subnet router announcing `192.168.1.0/24`** — the whole LAN reachable through one always-on node, no per-host client. IP forwarding persisted via `/etc/sysctl.d/99-tailscale.conf`; route approved + key-expiry disabled in the admin console. **WoL from the tailnet**: `install-wakeonlan.yaml` installs `wakeonlan` + a `/usr/local/bin/wake-earendil` wrapper (mirrors the operator's Mac alias) so samwise cold-starts the nightly-off hypervisor from anywhere. Validated end-to-end off-LAN: phone hotspot → services reachable only with Tailscale up; earendil powered down → `wake-earendil` from samwise woke it; remote WoL straight from the Mac correctly failed (the WiFi→wired magic-packet broadcast has to originate on-LAN, which samwise is). Possible later niceties, not required: self-hosted Headscale swap, a browser HTTP wake trigger.
- **Keycloak deployed as the IdP (replacing Authentik)** — chose Keycloak over Authentik for **CNCF-incubating governance** (vs single-vendor open-core) at comparable resource cost; pivoted *before* wiring any apps, so the switch was free. `keycloak.vingilot.internal` via the **Keycloak Operator** (vendored from `keycloak-k8s-resources` @ 26.6.2 under `gondor/apps/keycloak/operator/` — pinning the operator pins the server version). External CNPG Postgres (`keycloak-postgresql`, operator-default PG18); **no Redis** (embedded Infinispan on a single instance). Runs plain HTTP behind the vingilot Gateway — `http.httpEnabled` + `proxy.headers: xforwarded` + `ingress.enabled: false`; hand-managed CNPG PodMonitor. Initial admin auto-generated by the operator in the `keycloak-initial-admin` Secret. Authentik (briefly deployed, never used) was torn down and Flux-pruned. Follow-ons: wire app SSO (Grafana/Immich/Harbor OIDC) and manage realms/clients as code via `terraform-provider-keycloak`.
- **Harbor container + Helm registry deployed** — `harbor.vingilot.internal`, chart 1.19 (Harbor 2.15) via Flux in `gondor/apps/harbor/`. External Postgres (CNPG `harbor-postgresql`, no extensions → operator-default PG18) + per-app **Valkey** (ephemeral cache/broker — not the deprecated bundled redis) + bundled **Trivy** on from day one. `expose.type: clusterIP` behind the vingilot Gateway (TLS terminated there, `externalURL: https`); registry blobs on `nfs-scratch` (50Gi); admin password SOPS'd in `harbor-admin.yaml`; per-component resources hand-sized (chart ships none); hand-managed CNPG PodMonitor. Follow-ups: trust the tirion CA on Docker/containerd clients before `docker login`; add `trivy.gitHubToken` if vuln-DB pulls get rate-limited; replication mirroring + `oci://` Flux source (see CI/CD section).
- **CNPG metrics → Grafana: hand-managed PodMonitors (immich / harbor / auto-water)** — the CNPG dashboard was empty because nothing scraped the DB instances: they expose metrics on :9187, but the operator's only PodMonitor (`cnpg-system/cloudnative-pg`) covers the *controller*, not clusters. CNPG 1.29 deprecated the operator-managed `spec.monitoring.enablePodMonitor` flag ([PR #8753](https://github.com/cloudnative-pg/cloudnative-pg/pull/8753) — slated for removal; the project is pushing users to own their own), so rather than use it we added hand-managed `PodMonitor` CRs to each cluster (auto-water's lives in its own repo's `deploy/`). Each mirrors the operator's generated spec — notably `cnpg.io/podRole: instance` in the selector, whose omission is the metric-label-loss bug in [#8978](https://github.com/cloudnative-pg/cloudnative-pg/issues/8978). kube-prometheus-stack's match-all `podMonitorSelector` adopts them automatically; `cnpg_*` series confirmed flowing for all three. Codified the "always set this on a CNPG cluster" rule going forward.
- **Immich machine-learning enabled + server OOM fix** — flipped `machine-learning.enabled: true` (CPU image; model cache moved emptyDir → 10Gi `nfs-scratch` PVC so CLIP/face models survive restarts), unlocking smart search / face grouping / content-aware tagging. Enabling it surfaced a chronic OOM: the server pod (which also runs the microservices/transcode workers) spikes past its memory limit during ffmpeg/ffprobe jobs over the bulk-media tree — `container_memory_working_set_bytes` looked fine (~600Mi–1.2Gi) while `container_memory_max_usage_bytes` hit the 3Gi ceiling *between scrapes* (82 restarts/18d; the working-set graph hides sub-scrape spikes — trust `last_terminated_reason: OOMKilled` + max_usage). Bumped the server limit 3Gi→6Gi; gondor stays at 20Gi (earendil's 48Gi upgrade already removed the capacity ceiling — no guest rebalance needed). Deeper cure — scope the external library off the video/TV tree + cap Immich job concurrency in the admin UI — left as a follow-on.
- **auto-water sensor monitoring — live end-to-end on samwise** — the v2 sensor POC shipped: BH1750 (0x23, lux) + HDC3022 (0x44, temp/RH) + DS18B20 (1-Wire, soil temp) on the Pi 5 → a no-Flask Python poller (internal 30s loop + exec-heartbeat liveness, *not* a CronJob since gondor is down nightly) → CNPG Postgres (PG18, on gondor) → a Flux-loaded Grafana dashboard ("Auto-Water Sensors": a `grafana_dashboard` ConfigMap from the app repo + a SOPS'd Postgres datasource here). **Deploy manifests live in the app repo** (`swerdick/auto-water` `deploy/`) and Flux syncs them directly — homelab holds only the one-time `GitRepository`+`Kustomization` pointer; image is GHCR `linux/arm64` built in GHA, `lgpio` compiled from the `lg` C source (no Pi 5 wheel). Schema via roll-your-own up/down SQL migrations run as an initContainer, not inline DDL. DB-unreachable resilience is a 30-day in-memory retry buffer (1Gi pod limit) that backfills with original timestamps on reconnect — surviving a *pod restart* is the open "Durable on-disk retry buffer" item above. **Sensing only**; watering actuation (relay/solenoid + GPIO-out + watering logic) is still open — see the active "Auto-watering on samwise" entry.
- **XMP on earendil RAM — evaluated and declined** — the rated profile would lift memory bandwidth on paper, but on a **mixed kit** it may not run stably at the rated speed (looser-timing fallback, or no POST), and earendil is the always-on hypervisor for the whole fleet (gondor k3s incl. Postgres/CNPG) — so RAM instability has homelab-wide blast radius, DB corruption included. The only workload sensitive to RAM *speed* is CPU-bound gaming on anduril (k3s/containers are capacity/IO/CPU-bound, indifferent to DDR4 speed), and that gain is a few fps at most, partly capped by the Proton/VKD3D + GPU-sync + shader-comp parts that aren't memory-bound. Net: not worth the risk on this box; left at JEDEC default. Revisit only if the kit is matched or earendil stops being the homelab hypervisor.
- **anduril frame-pacing diagnosis + `mangohud` frame cap** — chased Elden Ring FPS dives (10-12 fps) end-to-end and ruled out everything infra-side: `steal=0` on every anduril vCPU during a dip (and powering `gondor` off changed nothing) killed the CPU-contention theory despite the 6+6 vCPU / 8-thread oversubscription; GPU clocked near max but ~30% util, `iowait=0`, NVENC light — nothing saturated. Root cause is a **forced-V-Sync staircase** (Elden Ring force-enables V-Sync + a 60 cap, no in-game toggle, so any frame-pacing hitch collapses to 60/N) layered on the **GTX 970 / i7-6700K hardware ceiling** in the open world (CPU/VKD3D-translation-bound ~25 fps). Codified `setup-gaming-tools.yaml` (wired into `site-anduril.yaml`) installing `mangohud` for an external cap via Steam launch option `MANGOHUD_CONFIG=fps_limit=30 mangohud %command%` — breaks the staircase; residual open-world stutter is the hardware. CPU-pinning the vCPUs was considered and declined (steal already 0).
...
- **earendil RAM upgrade 16 → 48 GiB** — new kit installed; 46.9 GiB seen by the host. Retires the long-standing constraint that two fixed-allocation 10 GiB VMs (gondor VM 140 + anduril VM 117, both `balloon: 0` for GPU passthrough) plus eregion couldn't coexist on 16 GiB — anduril no longer needs gondor/eregion stopped first to boot, and the "Audit guest CPU/memory + rebalance" item loses its hard ceiling. XMP profile evaluated and declined — mixed kit + hypervisor blast radius outweigh the gaming-only upside (see entry above).
