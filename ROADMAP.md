...
**Candidate role:** a remote / awkward-spot plant monitor-waterer running the same `yavanna` Python app from a venv + systemd unit (armhf from source, not the arm64 container samwise pulls), writing to the **same CNPG Postgres** as samwise — the `readings` table already keys on `sensor_id`, so a second node is free in the data model. samwise stays the main plant; hamfast is whenever a second plant in an odd spot materializes. No rush.
...
### Durable on-disk retry buffer for yavanna (post-SSD)

The poller's retry buffer — readings held while the CNPG sink is unreachable (gondor's nightly downtime, an extended trip) — is **in-memory only** as of [yavanna#12](https://github.com/swerdick/yavanna/pull/12): a 30-day time-based window backstopped by a 1 GiB pod memory limit. It rides out a long sink outage and flushes on reconnect, **but a pod restart or reschedule drops everything buffered.** Good enough while the window is days-long and restarts are rare; not the real answer for multi-week durability.

**Plan: back the buffer with on-disk storage** — a SQLite file (or append-only WAL) on a PVC, so buffered readings survive a pod restart and the memory limit stops being the effective retention bound. Gated on the **same USB SSD swap** the samwise yavannaing entry waits on (line above): until the SSD lands, a local-path PV would write to the SD card, which is exactly the flash-wear we're avoiding. Once it's in, this is a small change behind the existing `ReadingSink`/buffer seam (the buffer becomes a durable queue the poller drains), and it composes with the gRPC ingestion service in the hamfast thread above — a remote node would want the same local durability.
...
- **Anduril baseline convergence — alloy/observability on the Ubuntu CT** — the June rebuild had only run `setup-pseudo-user`, so anduril was dark in Grafana; applied `distribute-root-ca` + `install-alloy` (metrics + journald logs now ship to in-cluster Prometheus/Loki + Grafana Cloud — `up{instance="anduril"}=1` for the alloy + node jobs) + `setup-debian-base` + `install-unattended-upgrades`. anduril is Ubuntu 26.04 (the fleet's lone non-Debian LXC), so the Debian-first plays' `--check` failures were check-mode artifacts that cleared on a real run; also fixed `setup-debian-base` so its timezone default respects host_vars (TV clock stays `America/New_York`).
- **Anduril display normalization — native 4K + frame pacing** — retired the render-low + gamescope-upscale pattern whose FSR/NIS sharpen rang (the checkerboard): games render native `3840x2160` (gamescope 1:1, `-F linear` default), `HDMI-A-4` pinned `4K@60` at init (was drifting to the EDID-preferred `4K@30`), `mangohud fps_limit=60,30,0` locks pacing on the VRR-incapable 60 Hz panel, emulators default to 4K internal + non-sharpen filters (Eden→Bilinear, not FSR). Per-game Steam res + in-game FSR (e.g. Halo MCC) stay manual. `setup-anduril-session.yaml` + `install-anduril-emulators.yaml`.
...
- **CNPG metrics → Grafana via hand-managed PodMonitors** — the operator's PodMonitor only covers the controller, so each cluster (immich/harbor/yavanna) gets a hand-managed one; the selector must include `cnpg.io/podRole: instance` or labels are silently lost.
...
- **yavanna sensor monitoring — live on samwise** — BH1750 + HDC3022 + DS18B20 → a Python poller (30s loop, not a CronJob) → CNPG Postgres → a Flux-loaded Grafana dashboard; manifests in `swerdick/yavanna` `deploy/`. Sensing only.
...
- **CNPG Postgres backups → Backblaze B2** — all four clusters archive WAL continuously + take a staggered daily base backup via the Barman Cloud CNPG-I plugin (not the in-tree stanza, which CNPG removes in 1.30), to a dedicated bucket with a bucket-scoped key. Restore validated 2026-08-14: a full recovery into a scratch cluster took ~2 min and matched production exactly. `runbooks/postgres-restore.md`; `just pg-backup-{status,now,list}`. `volumeSnapshot` evaluated and deferred — see the local-tier entry above.
