...
### Harbor as upstream-registry proxy/cache (container images)

Every container image we use today is pulled live from Docker Hub / ghcr.io / quay.io / registry.k8s.io — each one a single point of failure for new pods (upstream rate-limits, removed-by-author, registry outages, supply-chain compromise) and an opaque trust boundary (we never scan upstream blobs before they land on the cluster). Harbor's **Proxy Cache** project type fronts an upstream OCI registry: first pull populates the cache, subsequent pulls hit Harbor, Trivy scans run on the cached copies.

**Scope:** one proxy-cache Harbor project per upstream (`dockerhub`, `ghcr`, `quay`, `registry-k8s`). Cluster-side rollout via k3s `/etc/rancher/k3s/registries.yaml` (templated by `install-k3s-server.yaml` + `install-k3s-agent.yaml`): `mirrors:` entries with `rewrite:` rules so `docker.io/library/postgres` resolves through Harbor's `dockerhub` proxy project transparently — **zero manifest churn** on existing HelmReleases / Deployments. `configs:` block references the tirion CA at `/usr/local/share/ca-certificates/vingilot-root-ca.crt` since containerd doesn't read the host trust store. Pull anonymous for the first cut (public Harbor projects); robot-account auth is a later hardening pass.

**Staged rollout to de-risk:**
1. Harbor side: one endpoint + one proxy project (`dockerhub`) to validate the cache works at all.
2. Manual `crictl pull harbor.vingilot.internal/dockerhub/library/alpine` test from gondor.
3. registries.yaml on **gondor only**; restart k3s; verify a pod's pull lands in the Harbor UI.
4. Roll the same registries.yaml to samwise.
5. Add remaining 3 upstreams (ghcr, quay, registry-k8s) in one fan-out.

**End state:** a Flux reconcile with every upstream registry offline still succeeds, Trivy has scanned every image we run, CVE drift becomes a queryable thing. (Helm-chart mirroring is a separate entry below — Harbor doesn't proxy-cache traditional chart repos, so the mechanism differs.)
...
### Anduril replatform: VM (GPU passthrough) → amdgpu-sharing LXC(s)

The GTX 970 was replaced with an **AMD RX 9070 XT** (RDNA4/gfx1201). Rather than re-solve vfio passthrough + driver pins a third time, anduril moves **off passthrough** to a **privileged LXC sharing the host's `amdgpu`** — which also unlocks a *second* LXC as a **k3s GPU worker for AI/ROCm** sharing the same card (cordon/drain hands it to gaming for heavy sessions). Subsumes the old "direct HDMI to the TV" goal (the gaming LXC drives the TV's HDMI via KMS *and* streams via Sunshine) and obsoletes the NVIDIA-cleanup items (the whole 970/NVENC stack is retiring). Plan: `~/.claude/plans/squishy-humming-thunder.md`.

**Phase 0 — foundation (DONE):** PVE 9.1.14→9.2.3, kernel pin retired (now on the 7.0 default — the pin only ever existed for the 970's vfio regression), `setup-amdgpu-host.yaml` replaces `setup-vfio-passthrough.yaml` so the AMD card binds `amdgpu` and exposes `/dev/dri` + `/dev/kfd`; NIC pinned to `lan0`. See Completed.

**Spike (CT 900) — DONE, see [[project-anduril-lxc-spike]]:** proved device passthrough, VAAPI **AV1/HEVC/H264** encode, DRM-master→TV (needs host fbcon off the AMD card), Plasma Wayland via seatd, and **Sunshine+Moonlight streaming the live desktop**. **One blocker found: Wayland input.** `libinput` enumerates zero input devices in a VT-less LXC (real *and* injected — not Sunshine-specific); KWin opens input via logind which wants a VT containers lack. X11 would fix it but Plasma drops X11 in 6.8 (Oct 2026).

**The fork — RESOLVED toward LXC.** Consumer RDNA4 has no SR-IOV and vfio passthrough is exclusive, so a gaming **VM** monopolizes the GPU and kills the shared k3s-AI path (unless the gaming VM also runs k3s — rejected, concern-mixing). LXC is the only route to shared gaming+AI on one GPU — and the input question that threatened it is now answered: **app-level input works** (a DualSense Edge paired on the host is read live inside the container via `/dev/input/event8`; Steam/games read `/dev/input` directly). Only the **Plasma desktop *pointer*** (libinput, needs a VT/logind) is blocked — irrelevant for a **Steam Big Picture / gamescope-session** couch console; RDP/SSH covers desktop admin. So **gaming=LXC is viable.** BT runs on the **host** (`AF_BLUETOOTH` is init-netns-only) with `ClassicBondedOnly=false`; the controller's `/dev/input` node passes into the LXC via bind-mount.

**Remaining:** (1) **solve/validate LXC input** (above) — gates the gaming-box decision; (2) **gaming LXC** — privileged Ubuntu 26.04, VAAPI, `setup-sunshine-amd.yaml` (ports autologin/linger/kde-inhibit/CSRF/audio, drops NVIDIA bits), games on a ZFS dataset, codify the proven device-passthrough + `sys:rw` + fbcon-off-AMD; VM kept as fallback until validated; (3) **k3s GPU-worker LXC** (Phase 2, **input-independent, unaffected**) — `/dev/kfd`+`/dev/dri` into pods (skip the AMD device plugin), GPU label+taint, ROCm. Tradeoff: shared-kernel `amdgpu` trades VM isolation for simplicity (a GPU hang can wedge the host).
...
- **earendil foundation refresh (anduril replatform Phase 0)** — PVE 9.1.14→9.2.3 (point upgrade, no Debian jump) + **kernel pin retired** (the 6.17 pin only ever existed for the GTX 970 vfio regression; now floating on the 7.0 default, fine-to-better for RDNA4) + `setup-amdgpu-host.yaml` **replaces** `setup-vfio-passthrough.yaml` so the AMD card binds `amdgpu` and exposes `/dev/dri` + `/dev/kfd` (host console stays on the iGPU). All gated behind a PBS backup of every guest; landed in one reboot. Lesson: verify the actual version delta before assuming — earendil was on the *latest major* (9.x), the real debt was the pin. Active replatform continues above.
- **earendil GPU swap recovery + NIC pinned to `lan0`** — the GTX 970→AMD 9070 XT swap renumbered the PCI bus and renamed the Realtek uplink `enp4s0`→`enp6s0`, stranding the host (vmbr0's bridge-port vanished → no carrier, host otherwise booting fine). Diagnosed from the console, repointed live, then made rename-proof: a systemd `.link` file (`setup-earendil-network.yaml`) pins the NIC by MAC to the stable name `lan0` so PCI re-enumeration can't strand it again; WoL re-arm already MAC-matched. Activated on the Phase 0 reboot.
- **HTTP→HTTPS redirect at the Gateway** — `gondor/infrastructure/instances/traefik/https-redirect.yaml`: a single cluster-wide HTTPRoute on the `web` listener with a `RequestRedirect` filter (scheme: https, 301), no hostname/path filter. Scales with every new HTTPS listener without per-service duplication. Cert-manager's HTTP-01 still works — its `/.well-known/acme-challenge/` routes win Gateway API's longest-PathPrefix tiebreaker over the catch-all.
