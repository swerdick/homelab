...
**Remaining:** (1) **spike** on a throwaway CT — prove device passthrough + a Wayland compositor taking DRM-master to light the TV from inside an LXC (the make-or-break) + Sunshine KMS capture + Bluetooth/DualSense; (2) **gaming LXC** — privileged Ubuntu 26.04, VAAPI encode, `setup-sunshine-amd.yaml` (ports autologin/linger/kde-inhibit/CSRF/audio, drops the NVIDIA bits), games on a ZFS dataset, VM kept as fallback until validated then retired; (3) **k3s GPU-worker LXC** — `/dev/kfd`+`/dev/dri` into pods (skip the AMD device plugin), GPU label+taint, ROCm. Accepted tradeoff: sharing the kernel `amdgpu` trades VM-level isolation for simplicity (a GPU hang can wedge the host, not just one VM). Encoder moves NVENC→VAAPI; Steam-Controller-2 best-effort USB passthrough still applies to the LXC.
